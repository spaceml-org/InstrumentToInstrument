...
The translation process is based on the core class ``itipy.translate.InstrumentToInstrument``.

.. autoclass:: itipy.translate.InstrumentToInstrument
...
is translated to SDO/HMI magnetograms. The translation is carried out by the ``itipy.translate.SOHOToSDO`` class.

.. autoclass:: itipy.translate.SOHOToSDO
...
simultaneously. The translation from STEREO to SDO is performed by the ``itipy.translate.STEREOToSDO`` class. Here, we translate between the EUVI and AIA instruments.

.. autoclass:: itipy.translate.STEREOToSDO
...
ITI can be used to complement the STEREO observations, by generating LOS magnetograms based on STEREO EUV filtergrams. This is performed by the ``itipy.translate.STEREOToSDOMagnetogram`` class.

.. autoclass:: itipy.translate.STEREOToSDOMagnetogram
...
to 0.15 arcsec pixels and use ITI to super resolve HMI observations by a factor of 4. This is performed by the ``itipy.translate.HMIToHinode`` class.

.. autoclass:: itipy.translate.HMIToHinode
...
The translation from PROBA2 to SDO is performed by the ``itipy.translate.PROBA2ToSDO`` class.

.. autoclass:: itipy.translate.PROBA2ToSDO
...
these dataproducts can be used for multi-viewpoint investigations. The translation is performed by the ``itipy.translate.FSIToAIA`` class.

.. autoclass:: itipy.translate.SolarOrbiterToSDO
...
and Solar Orbiter/HRI this allows for super-resolved full-sun AIA observations. This is performed by the ``itipy.translate.SDOToSolarOrbiter`` class.

.. autoclass:: itipy.translate.SDOToSolarOrbiter