...
swap_downloader = Proba2Downloader(base_path='swap')
swap_files = swap_downloader.downloadDate(date=datetime(2024, 5, 8, 15))

############################################################################################################################################################################
# Downloading AIA data
jsoc_email = os.environ["JSOC_EMAIL"]

sdo_downloader = SDODownloader(base_path='sdo', email=jsoc_email)
sdo_files = sdo_downloader.downloadDate(date=datetime(2024, 5, 8, 15))

############################################################################################################################################################################
# Glob the downloaded files

swap_files = sorted(glob.glob('/content/swap/174/*.fits', recursive=True))
sdo_files = sorted(glob.glob('/content/sdo/171/*.fits', recursive=True))
...
translator = SWAPToAIA(model_name='/content/iti-dataset/models/swap_to_aia_v0_4.pt')
