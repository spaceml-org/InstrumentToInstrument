...
parser = argparse.ArgumentParser(description='Train GOES to MSG translations')
parser.add_argument('--config', 
                    default='/home/anna.jungbluth/InstrumentToInstrument/config/goes_to_msg.yaml',
                    type=str, 
...
goes_path = data_config['A_path']
msg_path = data_config['B_path']
...
if 'A_norm_dir' and 'B_norm_dir' in norm_config:
    ...
    goes_norm = calculate_norm_from_metrics(norm_config['A_norm_dir'], split_dict=splits_dict['train'])
    msg_norm = calculate_norm_from_metrics(norm_config['B_norm_dir'], split_dict=splits_dict['train'])
else:
...
goes_bands = config['data']['A_bands']
goes_editors = [
...
data_module = ITIDataModule(goes_dataset, msg_dataset, goes_valid, msg_valid, **config['data'])
